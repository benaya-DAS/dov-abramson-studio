  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center gap-2">
        <Archive size={20} className="text-slate-500 dark:text-slate-400" />
        <h1 className="text-lg font-bold text-slate-900 dark:text-slate-100">ארכיון לוחות</h1>
      </div>

      {boards.length === 0 ? (
        <p className="text-sm text-slate-400 dark:text-slate-500">אין עדיין לוחות בארכיון.</p>
      ) : (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {boards.map((board) => (
            ...
            >
              ...
              <p className="mb-2 font-semibold text-slate-800 dark:text-slate-100">{board.name}</p>
