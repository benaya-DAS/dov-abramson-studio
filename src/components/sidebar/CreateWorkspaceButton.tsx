...
export default function CreateWorkspaceButton({ nextPosition }: { nextPosition: number }) {
    ...
    return (
      <div className="px-1">
        <input
          ...
          className="w-full rounded-md border border-brand-300 bg-white px-2 py-1.5 text-sm text-slate-900 outline-none dark:border-brand-700 dark:bg-night-800 dark:text-slate-100"
        />
  ...
  return (
    <button
      ...
      disabled={pending}
      className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-slate-500 hover:bg-slate-100 hover:text-brand-600 dark:text-slate-400 dark:hover:bg-night-800 dark:hover:text-brand-400"
    >
      <FolderPlus size={16} />
      מחלקה חדשה
    </button>
